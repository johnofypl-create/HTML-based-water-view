...
const PURPOSE = {
  ...
  'src/water/surface/heightField': '地形高度场 DataTexture 生成与 UV 映射（物理水/着色器采样用）',
  'src/water/Water': '水面组件（装配 Plane + 波位移 + 泡沫材质）',
  'src/water/surface/gerstner': 'Gerstner 波单一事实源（TS 端采样，供 CPU 发射器/物理用）',
  'src/water/surface/gerstner.glsl': 'Gerstner 波 GLSL 生成（与 gerstner.ts 同源，供顶点位移）',
  'src/water/surface/waterSurface': '水面高度统一抽象 sampleWaterSurface（未来接高度场灌水只改此处）',
  'src/water/foam/waterMaterial': '海面着色器材质工厂（Gerstner 位移 + Jacobian 白帽 + 岸线湿边）',
  'src/water/foam/SprayParticles': '轻量 GPU 飞溅粒子层（环形缓冲 + 闭式弹道 + 三触发源）',
  'src/water/state/splashTargets': '溅水目标注册表（礁石/岛屿登记，供拍浪判定）',
  'src/water/foam/sprayShader': '飞溅粒子顶点/片段着色器（弹道积分 + 软圆点）',
  'src/water/physics/shallowWater': '高度场浅水 Virtual Pipes 参考求解器（CPU，物理水 P0）',
...
const AIEDIT = {
  'src/water/foam/waterMaterial': '调泡沫密度/颜色 → 改 fragment 的 applyFoam 与 uFoam*；调波形 → 改 gerstner.glsl.ts；调昼夜响应 → 改 updateWaterMaterial 中 lightingState.* 读取',
  'src/water/surface/gerstner': '调波形/风向 → 改 GERSTNER_WAVES 数组与 primaryWindDir（与 gerstner.glsl.ts 同源）',
  'src/water/surface/gerstner.glsl': '调波形/风向 → 改本文件（与 gerstner.ts 同源）；数值改动属算法层，本次重构不动',
  'src/water/foam/SprayParticles': '调粒子数/寿命/尺寸 → 改 config/spray.ts；调触发逻辑 → 改本文件 spawn 段（波峰/地形事件/礁石拍浪）',
  'src/water/physics/shallowWater': '调物理稳定性 → 改 K/dt 与 outBuffer 钳制（见本文件注释）；数值改动属算法层，本次重构不动',
  ...
  'src/water/surface/waterSurface': '接入高度场灌水 → 改 sampleWaterSurface 的 surfaceY = T + h（未来 P1）',
  'src/state/splashBus': '触发地形改造溅水 → splashBus.emit({pos,intensity})，无需改 SprayParticles',
  'src/water/state/splashTargets': '新增溅水目标（礁石/岛屿）→ registerSplashTarget({pos,radius,...})',
}
